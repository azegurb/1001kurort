...
import DoctorsArticles from '../forms/DoctorsArticles'
import DoctorsAskDoctor from '../forms/DoctorsAskDoctor'
import MessagesHandler from '../MessagesHandler'

const MenuItems =	[
						{id : '0', label : ['My profile','Мой профиль'] },
						{id : '1', label : ['Blog','Блог'] },
						{id : '2', label : ['Ask doctor','Спросить врача'] },
...
export default class DoctorDashboard extends Component {
    ...
    componentWillMount() {
        ...
        if ( _.isEmpty(profile.user) ) {
            history.replace({ pathname: '/auth/doctor' });
        }
    }
				...
				<div>
					<Row>
						<Drawer
							...
							<Col> 
								{ languageId === 0 ? 'ID' : 'ИД' } : { user.user_id } 
							</Col>
							<List style={{ marginTop : 50 }}>
								
									<ListItem value={0} primaryText={ languageId === 0 ? 'My profile' : 'Мой профиль' } onClick={ () => this.handleNestedMenuItems(0) }/>
									<ListItem value={1} primaryText={ languageId === 0 ? 'Blog' : 'Блог' } onClick={ () => this.handleNestedMenuItems(1) }/>
									<ListItem value={2} primaryText={ languageId === 0 ? 'Ask doctor' : 'Спросить врача' } onClick={ () => this.handleNestedMenuItems(2) }/>
						...
						<Col  xs={10} offset={{ xs : 1 }}>
							...
							{ this.state.activeProfileMenuItem === 0 && <DoctorFormProfileInfo languageId={languageId}  /> }							
							{ this.state.activeProfileMenuItem === 1 && <DoctorsArticles data= {this.props.profile.user} languageId={languageId}  /> }
							{ this.state.activeProfileMenuItem === 2 && <DoctorsAskDoctor data= {this.props.profile.user} languageId={languageId}  /> }
							{ this.state.activeProfileMenuItem === 3 && <MessagesHandler data= {this.props.profile.user} languageId={languageId} account_type={2}/> }
