...
export default class HotelsRoomDetails extends Component {
	...
	constructor(props) {
		...
		this.state = Object.assign({ rooms: [], roomsNames: [] }, initialState)

		this.axiosGetRoomsNames = ::this.axiosGetRoomsNames;
		this.axiosGetRoomsDetails = ::this.axiosGetRoomsDetails;
	...
	axiosGetRoomsNames() {
		...
		}).then( response => this.setState({ rooms: response.data.data }) )
	}

	axiosGetRoomsDetails() {
		...
		axios.get('/api/items-details-names').then( responseNames =>{
			...
			axios
				.get('/api/profile/hotel/rooms-props', { 
					params: {
						items_id: this.state.activeRoom
					}
				})
				.then( responseProps => {
	...
	handleSaveChanges() {
		axios
			.post('/api/profile/hotel/rooms-props/update',
				{
					item_id: this.state.activeRoom,
						...
						freeList.push(
							<Col xs={4} style={ item.checked && { background: 'rgba(73, 196, 7, 0.11)' } }>
								<Checkbox 
									...
									onCheck={ (e, checked ) => this.handleItemChange(e, checked, listData) } />
							</Col>
						)  
					:
						paidList.push(
	...
	handleItemChange( e, value, listData ) {
	...
	let listData = this.state.listData,
		...
		if(_.find( listData, item => { return item.label[0] === nameService || item.label[1] === nameService  ? true : false }) ){
			...
			listData.push({ label : [ nameService, nameService], id : nameService, free: categoryService  ? true : false, available: true })
		...
		return(
				<div>
					<Row>
						<Col>
							<SelectField
								floatingLabelText={ languageId === 0 ? 'Select room' : 'Выберите комнату' }
								value={this.state.activeRoom}
								onChange={ (e,i, value) => this.setState({ activeRoom: value }, () => this.axiosGetRoomsDetails() ) }
							>
								{
									this.state.rooms.map( item =>
										<MenuItem value={item.room_id} primaryText={ languageId === 0 ? item.sname : item.sname_rus } />
									)
								}
							</SelectField>
						</Col>
					...
					{
						this.state.activeRoom && ( ::this.buildList(this.state.listData , languageId) )
						...
						>
							<Row style={{ height: 86, marginLeft : 0, marginRight: 0  }}>
								...
								<Col xs={3}>
									...
									>
										<MenuItem value='free' primaryText={ languageId === 0 ? 'Free' : 'Бесплатно' } />
										<MenuItem value='paid' primaryText={ languageId === 0 ? 'Paid' : 'Платно' } />
									</SelectField>								
								</Col>								
